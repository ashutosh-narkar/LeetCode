...
def IPv4(ip):
    ...
    for item in ary:
        if not item.isdigit() or not 0 <= int(item) <= 255 or (item[0] == '0' and len(item) > 1):
            return False

    return True


def IPv6(ip, hex_digits):
    ary = ip.split(":")
    ...
    for item in ary:
        if len(item) == 0 or len(item) > 4 or not is_hex(item, hex_digits):
            return False
    return True


def is_hex(s, hex_digits):
    for ch in s:
